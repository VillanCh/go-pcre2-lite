...
- **更快、更省分配** —— 在所有基准上都快于 `dlclark/regexp2`（最高约 5.9 倍）、
...
| 构造 | 示例 | 处理方式 |
|---|---|---|
| 变长后向断言 | `(?<=a+)b`、`(?<="text":\s*")` | PCRE2 10.43 原生支持有界变长后向断言；后向断言内的无界量词被收紧为 `{n,512}` 以便编译与匹配（超过 512 次重复不匹配） |
...
drop-in 层在所有基准上都快于 `dlclark/regexp2`，低层字节 API 更快（1.2x–6.2x）。

| 场景 | dlclark | drop-in | low-level | 提速 | drop-in 分配 |
|---|---|---|---|---|---|
| 布尔匹配，短字符串 | 6471 ns | 1099 ns | 1088 ns | **5.9x** | 0 B / 0 |
| 布尔匹配，100 KB 输入 | 25.9 ms | 4.51 ms | 4.49 ms | **5.7x** | 0 B / 0 |
| 含反向引用的匹配 | 391 ns | 186 ns | 183 ns | **2.1x** | 0 B / 0 |
| 回溯密集，32 KB | 20.4 ms | 11.2 ms | 11.1 ms | **1.8x** | 0 B / 0 |
| Unicode `\p{Han}`，8 KB | 15.6 µs | 12.2 µs | 4.65 µs | 1.3x | 0 B / 0 |
| 含 6 个捕获的 Find | 1129 ns | 908 ns | 677 ns | 1.2x | 752 B / 7 |
| 编译（复杂模式） | 10.9 µs | ~3.0 µs | 2.95 µs | **3.7x** | 1.5 KB / 17 |
| 全量 Find-all，670 个匹配 | 380 µs | 139 µs | 61 µs | **2.7x**（ll 6.2x） | 193 KB / 2004 |
| 全量 Find-all，3 万个匹配 | 6.29 ms | 5.38 ms | 2.94 ms | **1.2x**（ll 2.1x） | 7.8 MB / 90k |
...
（约 59 倍），drop-in 层从 **170 ms 降到 5.4 ms**，二者现在都**快于 `dlclark`
（6.3 ms）和 Go 的 `regexp`（5.8 ms）**。
...
- **以 PCRE2 10.42 为基准真值：** 匹配结果在 PCRE2 自带 `testoutput2`/`testoutput4`
  语料上达成 761/761（8 位）与 1347/1347（UTF）一致；编译接受/拒绝在 1080/1080 接受、
  286/286 拒绝上一致。
